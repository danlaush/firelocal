...
export type Subscription =
  | { type: 'doc'; subId: string; path: string; notify: (snap: RawSnapshot) => void }
  | { type: 'query'; subId: string; collectionPath: string; constraints: import('./types').Constraint[]; notify: (snap: RawSnapshot) => void };
...
export function createContext(db: DbLike): HandlerContext {
...
export function handleMessage(
  ...
  try {
    switch (msg.type) {
      ...
      case 'SUBSCRIBE_DOC': {
        const notify = (snap: RawSnapshot) => respond({ type: 'SNAPSHOT', subId: msg.subId, snapshot: snap });
    ...
    if ('id' in msg) {
      respond({ type: 'ERROR', id: (msg as { id: string }).id, error: String(err) });
    }
  }
}

function notifySubscribers(ctx: HandlerContext, affectedPath: string): void {
  const { db, subscriptions } = ctx;
